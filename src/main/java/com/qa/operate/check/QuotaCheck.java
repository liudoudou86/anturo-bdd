...
public class QuotaCheck {
    ...
    public void CheckProductSetData(DataTable dataTable, Response interfaceResult) {
        ...
        Assertions.assertThat(testcaseData).as("月度临时超额度金额校验-产品集合数据校验无误").containsExactlyInAnyOrderElementsOf(interfaceData);
        log.info("月度临时超额度金额校验-产品集合数据校验无误");
    }

    @Step(value = "超额度金额校验-品种")
    public void CheckProductVarietyData(DataTable dataTable, Response interfaceResult) {
        ...
        Assertions.assertThat(testcaseData).as("月度临时超额度金额校验-品种数据校验无误").containsExactlyInAnyOrderElementsOf(interfaceData);
        log.info("月度临时超额度金额校验-品种数据校验无误");
    }

    @Step(value = "超额度金额校验-临时信用申请")
    public void CheckTemporaryCreditData(DataTable dataTable, List<Map<String, Object>> sqlResult) {
        ...
        Assertions.assertThat(testcaseData).as("月度临时超额度金额校验-临时信用申请数据校验无误").containsExactlyInAnyOrderElementsOf(sqlData);
        log.info("月度临时超额度金额校验-临时信用申请数据校验无误");
    }

    @Step(value = "超额度金额校验-信用流水")
    public void CheckCreditJounalData(DataTable dataTable, List<Map<String, Object>> sqlResult) {
        ...
        Assertions.assertThat(testcaseData).as("月度临时超额度金额校验-信用流水数据校验无误").containsExactlyInAnyOrderElementsOf(sqlData);
        log.info("月度临时超额度金额校验-信用流水数据校验无误");
    }
}
