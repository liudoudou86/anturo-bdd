...
import com.anturo.qa.center.order.bdd.order.backgrounds.ProductBackgroundData01;
...
@Slf4j
public class ProductPriceCustomerPreDiscountSpec extends ProductBackgroundData01 implements En {
	...
	public ProductPriceCustomerPreDiscountSpec() {
		super();
		this.scenario01();
		this.scenario02();
		this.scenario03();
	...
	private void scenario01() {
		Given("[客户预折商品获取单价01]当前存在如下客户信息", (DataTable dataTable) -> {
		...
		When("[客户预折商品获取单价01]当前存在如下商品价格信息", (DataTable dataTable) -> {
		...
		Then("[客户预折商品获取单价01]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				...
				Response apiResult = orderApiPrediscountFetchPrice01.customerPreDiscountFetchProductPrice(OrderCustomerPreDiscountFetchProductPriceDTO.builder()
						.orderType("STANDARD")
	...
	private void scenario02() {
		Given("[客户预折商品获取单价02]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价02]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价02]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario03() {
		Given("[客户预折商品获取单价03]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价03]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价03]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario04() {
		Given("[客户预折商品获取单价04]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价04]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价04]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario05() {
		Given("[客户预折商品获取单价05]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价05]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价05]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario06() {
		Given("[客户预折商品获取单价06]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价06]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价06]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario07() {
		Given("[客户预折商品获取单价07]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价07]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价07]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario08() {
		Given("[客户预折商品获取单价08]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价08]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价08]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario09() {
		Given("[客户预折商品获取单价09]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价09]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价09]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario10() {
		Given("[客户预折商品获取单价10]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价10]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价10]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario11() {
		Given("[客户预折商品获取单价11]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价11]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价11]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario12() {
		Given("[客户预折商品获取单价12]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价12]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价12]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
	...
	private void scenario13() {
		Given("[客户预折商品获取单价13]当前存在如下客户信息", (DataTable dataTable) -> {
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
				saleOrgSapCode = Optional.ofNullable(caseData.get("saleOrgSapCode")).orElse("");
		...
		When("[客户预折商品获取单价13]当前存在如下商品价格信息", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String dimensionCode = Optional.ofNullable(caseData.get("dimensionCode")).orElse("");
				String dimensionPriority = Optional.ofNullable(caseData.get("dimensionPriority")).orElse("");
				String distributionChannelCode = Optional.ofNullable(caseData.get("distributionChannelCode")).orElse("");
		...
		Then("[客户预折商品获取单价13]预折添加商品时获取价格应该如下", (DataTable dataTable) -> {
			dataTable.asMaps().stream().forEach(caseData -> {
				String itemCode = Optional.ofNullable(caseData.get("itemCode")).orElse("");
				String preType = Optional.ofNullable(caseData.get("preType")).orElse("");
				String price = new BigDecimal(caseData.get("price")).setScale(2, RoundingMode.HALF_UP).toString();
