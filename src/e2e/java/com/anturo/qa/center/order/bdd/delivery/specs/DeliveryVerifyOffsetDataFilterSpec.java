...
@Slf4j
public class DeliveryVerifyOffsetDataFilterSpec implements En {
	...
	public DeliveryVerifyOffsetDataFilterSpec() {
		super();
		this.scenario01();
		this.scenario02();
		this.scenario03();
	...
	private void scenario01() {
		Given("[交货单校验冲销过滤01]当前存在如下客户信息", (DataTable dataTable) -> {
		...
		And("[交货单校验冲销过滤01]SAP发送交货单数据如下", (DataTable dataTable) -> {
		...
		And("[交货单校验冲销过滤01]SAP发送交货单10行冲销数据", (DataTable dataTable) -> {
		...
		Then("[交货单校验冲销过滤01]交货单过滤重复数据正常入库", (DataTable dataTable) -> {
	...
	private void scenario02() {
		Given("[交货单校验冲销过滤02]当前存在如下客户信息", (DataTable dataTable) -> {
			createDate = new SimpleDateFormat("yyyyMMdd").format(System.currentTimeMillis());
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤02]SAP发送交货单数据如下不发送", (DataTable dataTable) -> {
			deliveryNumber = "220" + RandomUtil.randomNumbers(7);
			materialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		And("[交货单校验冲销过滤02]SAP发送交货单10行冲销数据", (DataTable dataTable) -> {
			writeOffDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			writeOffMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
	...
	private void scenario03() {
		Given("[交货单校验冲销过滤03]当前存在如下客户信息", (DataTable dataTable) -> {
			createDate = new SimpleDateFormat("yyyyMMdd").format(System.currentTimeMillis());
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤03]SAP发送交货单10行正向和冲销数据同时接收", (DataTable dataTable) -> {
			deliveryNumber = "220" + RandomUtil.randomNumbers(7);
			writeOffDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			materialNumber = "440" + RandomUtil.randomNumbers(7);
			writeOffMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			dataTable.asMaps().stream().forEach(
					caseData -> {
						String orderItem = caseData.get("orderItem");
						batchNum = caseData.get("batchNum");
						orderApiDelivery01.createAndWriteOffPositiveDeliveryShipment(OrderDeliveryDTO.builder()
										.date(createDate)
										.deliverNo(deliveryNumber)
										.writeOffDeliverNo(writeOffDeliveryNumber)
										.orderItem(orderItem)
										.batchNum(batchNum)
										.eccid(eccId)
										.orderItemQuantityTotal(batchNum)
		...
		Then("[交货单校验冲销过滤03]数据正常接收入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
	...
	private void scenario04() {
		Given("[交货单校验冲销过滤04]当前存在如下客户信息", (DataTable dataTable) -> {
			createDate = new SimpleDateFormat("yyyyMMdd").format(System.currentTimeMillis());
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤04]SAP发送交货单数据如下", (DataTable dataTable) -> {
			deliveryNumber = "220" + RandomUtil.randomNumbers(7);
			materialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		Then("[交货单校验冲销过滤04]交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
		...
		When("[交货单校验冲销过滤04]创建退货订单且下发SAP", (DataTable dataTable) -> {
		...
		And("[交货单校验冲销过滤04]SAP发送退货交货单数据如下", (DataTable dataTable) -> {
		...
		And("[交货单校验冲销过滤04]SAP发送退货冲销交货单数据如下", (DataTable dataTable) -> {
		...
		And("[交货单校验冲销过滤04]SAP发送退货交货单重复数据如下", (DataTable dataTable) -> {
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
					caseData -> {
						String returnOrderItem = caseData.get("returnOrderItem");
		...
		Then("[交货单校验冲销过滤04]退货交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
	...
	private void scenario05() {
		Given("[交货单校验冲销过滤05]当前存在如下客户信息", (DataTable dataTable) -> {
			createDate = new SimpleDateFormat("yyyyMMdd").format(System.currentTimeMillis());
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤05]SAP发送交货单数据如下", (DataTable dataTable) -> {
			deliveryNumber = "220" + RandomUtil.randomNumbers(7);
			materialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		Then("[交货单校验冲销过滤05]交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
		...
		When("[交货单校验冲销过滤05]创建退货订单且下发SAP", (DataTable dataTable) -> {
			List<ReturnOrderDetailRespDTO> returnOrderDetailRespDTOS = new ArrayList<>();
			List<AcceptDetailReqDTO> acceptDetailReqDTOS = new ArrayList<>();
			List<String> refundOrderItems = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤05]SAP发送退货交货单数据如下不发送", (DataTable dataTable) -> {
			returnDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			returnMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		And("[交货单校验冲销过滤05]SAP发送退货冲销交货单数据如下", (DataTable dataTable) -> {
			writeOffReturnDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			writeOffReturnMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		And("[交货单校验冲销过滤05]SAP发送退货交货单重复数据如下", (DataTable dataTable) -> {
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
					caseData -> {
						String returnOrderItem = caseData.get("returnOrderItem");
		...
		Then("[交货单校验冲销过滤05]退货交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
	...
	private void scenario06() {
		Given("[交货单校验冲销过滤06]当前存在如下客户信息", (DataTable dataTable) -> {
			createDate = new SimpleDateFormat("yyyyMMdd").format(System.currentTimeMillis());
			createTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
			domainDataTable = dataTable;
			domainDataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤06]SAP发送交货单数据如下", (DataTable dataTable) -> {
			deliveryNumber = "220" + RandomUtil.randomNumbers(7);
			materialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		Then("[交货单校验冲销过滤06]交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
		...
		When("[交货单校验冲销过滤06]创建退货订单且下发SAP", (DataTable dataTable) -> {
			List<ReturnOrderDetailRespDTO> returnOrderDetailRespDTOS = new ArrayList<>();
			List<AcceptDetailReqDTO> acceptDetailReqDTOS = new ArrayList<>();
			List<String> refundOrderItems = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
		...
		And("[交货单校验冲销过滤06]SAP发送退货交货单数据如下", (DataTable dataTable) -> {
			returnDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			returnMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			List<OrderDeliveryDTO> orderDeliveryDTOS = new ArrayList<>();
			dataTable.asMaps().stream().forEach(
		...
		And("[交货单校验冲销过滤06]SAP发送交货单10行正向和冲销数据同时接收", (DataTable dataTable) -> {
			writeOffReturnDeliveryNumber = "220" + RandomUtil.randomNumbers(7);
			writeOffReturnMaterialNumber = "440" + RandomUtil.randomNumbers(7);
			dataTable.asMaps().stream().forEach(
					caseData -> {
						String returnOrderItem = caseData.get("returnOrderItem");
						String deliveryBatchNum = caseData.get("deliveryBatchNum");
						orderApiDelivery01.createAndWriteOffRefundDeliveryShipment(OrderDeliveryDTO.builder()
										.date(createDate)
										.deliverNo(returnDeliveryNumber)
										.writeOffDeliverNo(writeOffReturnDeliveryNumber)
										.orderItem(returnOrderItem)
										.batchNum(deliveryBatchNum)
										.eccid(returnEccId)
										.orderItemQuantityTotal(deliveryBatchNum)
										...
										.refundOrderNo(returnOrderNo)
										.materialNumber(returnMaterialNumber)
										.writeOffMaterialNumber(writeOffReturnMaterialNumber)
										.build())
								.then()
								.body(Matchers.containsString("<data>true</data>"));
					}
		...
		Then("[交货单校验冲销过滤06]退货交货单过滤重复数据正常入库", (DataTable dataTable) -> {
			List<Map<String, String>> caseDataMap = new ArrayList<>();
			dataTable.asMaps().forEach(caseData -> {
				Map<String, String> processedData = new HashMap<>();
				processedData.put("item_id", Optional.ofNullable(caseData.get("orderItem")).orElse(""));
